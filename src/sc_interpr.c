...
static void
xCleanUpLoops(struct sc_ProcessBlock *sc_pb)
{
	TpsLoop prev, this;
...
static int
findendloop(TscOperand label, struct sc_ProcessBlock *sc_pb)
{
	TscOperand operand;
	TscOpcode opcode;
...
static int
findloop(TpsLoop *ret, TscOperand label, struct sc_ProcessBlock *sc_pb)
{
	TpsLoop walk;

	for (walk = sc_pb->loops; walk != NULL; walk = walk->l_next) {
		if (walk->l_label == label) {
            break;
        }
	}
	if (walk == NULL) {
		TpsLoop newloop = (TpsLoop)malloc(sizeof(TsLoop));
		if (newloop == NULL) {
			err_nomem();
			return -1;
		}
		...
		newloop->l_index = 0;
		newloop->l_retaddr = sc_pb->pc
                             - (sizeof(TscOpcode) + sizeof(TscOperand));
...
static void
removeloop(TpsLoop this, struct sc_ProcessBlock *sc_pb)
{
...
static int
init(object *self, struct sc_ProcessBlock *sc_pb)
{
	object *string = gettupleitem(self, STUBC);
	int datasize;
...
static void reinit(struct sc_ProcessBlock *sc_pb)
{
	sc_pb->bp = 0;

	for (int i = 0; i < sc_pb->sp; i++) {
		DECREF(sc_pb->stack[i]);
	}
	sc_pb->sp = 0;
}

static void UnInit(object *self, struct sc_ProcessBlock *sc_pb)
{
	free(sc_pb->buffer);
	reinit(sc_pb);
...
static object *
stacktop(struct sc_ProcessBlock *sc_pb)
{
	if (sc_pb->sp == 0) {
...
static int
xPush(object *element, struct sc_ProcessBlock *sc_pb)
{
	if (sc_pb->sp >= STKSIZE) {
...
/* Perform the RPC. */
static int
xTrans(object *self, TscOperand cmd, struct sc_ProcessBlock *sc_pb)
{
	short ret;
	object *capobj;
...
/* Test the tuple size. */
static int
xTTupleS(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *tuple;
...
/* Unpack a tuple */
static int
xUnpack(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
...
static int
xAilword(TscOperand headerfield, struct sc_ProcessBlock *sc_pb)
{
	switch(headerfield) {
		case H_EXTRA:
...
static int
xStringS(struct sc_ProcessBlock *sc_pb)
{
	object *size, *string;
	int s;
...
static int
xListS(struct sc_ProcessBlock *sc_pb)
{
	object *size, *list;
	int s;
...
static int
xPutFS(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *string;
	char *str;
...
static int
xTStringSeq(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *string;
...
static int
xTStringSlt(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *string;
...
static int
xTListSeq(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *list;
...
static int
xTListSlt(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *list;
...
static int
xPutVS(struct sc_ProcessBlock *sc_pb)
{
	object *string;
	char *str;
	int size;
...
static int
xLoopPut(TscOperand label, struct sc_ProcessBlock *sc_pb)
{
	TpsLoop this;
	object *list, *element;
...
static int
xLoopGet(TscOperand label, struct sc_ProcessBlock *sc_pb)
{
	TpsLoop this;
...
static int
xEndLoop(TscOperand label, struct sc_ProcessBlock *sc_pb)
{
	TpsLoop this;
...
static int
xPutI(TscOperand flags, struct sc_ProcessBlock *sc_pb)
{
	object *integer;
...
static int
xPutC(struct sc_ProcessBlock *sc_pb)
{
	capability cap;
	object *capobj;
...
static int
xDup(TscOperand n, struct sc_ProcessBlock *sc_pb)
{
	if ((int)n > sc_pb->sp) {
		object *i;
...
static int
xPop(TscOperand n, struct sc_ProcessBlock *sc_pb)
{
	if ((sc_pb->sp - (int)n) < 0) {
...
static int
xAlign(TscOperand n, struct sc_ProcessBlock *sc_pb)
{
...
static int
xPack(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
...
static int
xGetVS(struct sc_ProcessBlock *sc_pb)
{
	...
	if ((string = newsizedstringobject(&sc_pb->buffer[sc_pb->bp], size))
        == NULL)
    {
		return -1;
    }
...
static int
xGetFS(TscOperand size, struct sc_ProcessBlock *sc_pb)
{
	object *string;
	...
	if ((string = newsizedstringobject(&sc_pb->buffer[sc_pb->bp], (int)size))
        == NULL)
    {
		return -1;
    }
...
static int
xGetI(TscOperand flags, struct sc_ProcessBlock *sc_pb)
{
	object *integer;
	long i;
...
static int
xGetC(TscOperand flags, struct sc_ProcessBlock *sc_pb)
{
...
static int
xEqual(struct sc_ProcessBlock *sc_pb)
{
	object *int1, *int2;
...
object *
sc_interpreter(object *self, object *args)
{
	TscOpcode opcode;
	TscOperand operand;
	int datasize, ret;
	...
	while (sc_pb.pc < datasize) {
		...
		switch(opcode) {
			...
			case PushI:
				{
					object *element = newintobject((long)operand);
					if (element == NULL) {
						UnInit(self, &sc_pb);
						return NULL;
					}
