...
object *
newmethodobject(char *name, method meth, object *self)
{
...
static void
meth_print(methodobject *m, FILE *fp, int flags)
{
	if (m->m_self == NULL) {
...
meth_repr(methodobject *m)
{
	...
	if (m->m_self == NULL) {
		...
		sprintf(buf, "<built-in method '%.80s' of some %.80s object>",
				m->m_name, m->m_self->ob_type->tp_name);
    }
