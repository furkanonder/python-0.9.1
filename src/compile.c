...
static object *
code_getattr(codeobject *co, char *name)
{
	return getmember((char *)co, code_memberlist, name);
}

static void
code_dealloc(codeobject *co)
{
	XDECREF(co->co_code);
...
newcodeobject(object *code, object *consts, object *names, char *filename)
{
	...
	if (code == NULL || !is_stringobject(code) || consts == NULL
        || !is_listobject(consts) || names == NULL || !is_listobject(names))
    {
		err_badcall();
		return NULL;
	}
	/* Make sure the list of names contains only strings */
	for (int i = getlistsize(names); --i >= 0; ) {
...
static int
com_add(struct compiling *c, object *list, object *v)
{
...
static int
com_addconst(struct compiling *c, object *v)
{
	return com_add(c, c->c_consts, v);
}

static int
com_addname(struct compiling *c, object *v)
{
...
static void
com_apply_subscript(struct compiling *c, node *n)
{
	REQ(n, subscript);
...
com_comparison(struct compiling *c, node *n)
{
	...
	****************************************************************/
	...
	for (i = 2; i < NCH(n); i += 2) {
		...
		if (op == BAD) {
			err_setstr(SystemError, "com_comparison: unknown comparison op");
			c->c_errors++;
		}
...
static void
com_list(struct compiling *c, node *n)
{
	/* exprlist: expr (',' expr)* [',']; likewise for testlist */
	if (NCH(n) == 1) {
		com_node(c, CHILD(n, 0));
...
static void
com_assign_slice(struct compiling *c, node *n, int assigning)
{
	com_slice(c, n, assigning ? STORE_SLICE : DELETE_SLICE);
}

static void
com_assign_subscript(struct compiling *c, node *n, int assigning)
{
	com_node(c, n);
...
com_assign(struct compiling *c, node *n, int assigning)
{
	...
	for (;;) {
		switch (TYPE(n)) {
			...
			case not_test:
				if (NCH(n) > 1) {
					err_setstr(TypeError, "can't assign to operator");
					c->c_errors++;
					return;
				}
				n = CHILD(n, 0);
			...
			case factor: /* ('+'|'-') factor | atom trailer* */
				...
				if (NCH(n) > 1) { /* trailer present */
					...
					com_assign_trailer(c, CHILD(n, i), assigning);
					return;
				}
				n = CHILD(n, 0);
				break;
...
{
	int break_anchor = 0, anchor = 0, begin;
...
{
	int finally_anchor = 0, except_anchor = 0;
	...
	if (except_anchor) {
		...
		for (int i = 3; i < NCH(n)
            && TYPE(ch = CHILD(n, i)) == except_clause; i += 3)
        {
			/* except_clause: 'except' [expr [',' expr]] */
			if (except_anchor == 0) {
				err_setstr(TypeError, "default 'except:' must be last");
				c->c_errors++;
				break;
			}
			except_anchor = 0;
